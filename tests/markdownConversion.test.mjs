...
const successCallout = obsidianToNotionMarkdown("> [!success] Done\n> It worked.");
assert.equal(successCallout.markdown, '<callout icon="✅" color="green_bg">\n\t**Done**\n\tIt worked.\n</callout>');
assert.equal(notionToObsidianMarkdown(successCallout.markdown).markdown, "> [!success] Done\n> It worked.");
...
const unknownCallout = '<callout icon="🔍" color="blue_bg">\n\t**Custom title**\n\tBody with **bold**, `code`, and [a link](https://example.com).\n\t- parent\n\t\t- nested\n</callout>';
const unknownCalloutObsidian = notionToObsidianMarkdown(unknownCallout);
assert.equal(unknownCalloutObsidian.status, "success");
assert.equal(unknownCalloutObsidian.markdown, '> [!note] Custom title\n> <!-- llm-wiki-sync:callout:v1 icon=%F0%9F%94%8D color=blue_bg -->\n> Body with **bold**, `code`, and [a link](https://example.com).\n> - parent\n> \t- nested');
const unknownCalloutBack = obsidianToNotionMarkdown(unknownCalloutObsidian.markdown);
assert.equal(unknownCalloutBack.status, "success");
assert.equal(unknownCalloutBack.markdown, unknownCallout);
for (const [icon, color] of [["⚠️", "red_bg"], ["🔴", "red_bg"], ["🧪", "purple_bg"], ["🦄", "pink_bg"]]) {
  const source = `<callout icon="${icon}" color="${color}">\n\tbody\n</callout>`;
  const pulled = notionToObsidianMarkdown(source);
  assert.equal(pulled.status, "success");
  assert.equal(obsidianToNotionMarkdown(pulled.markdown).markdown, source);
}
assert.equal(notionToObsidianMarkdown("<callout>\n\tbody\n</callout>").status, "unsafe");
assert.equal(areObsidianAndNotionBodiesEquivalent(unknownCalloutObsidian.markdown, unknownCallout), true);
assert.equal(obsidianToNotionMarkdown('> [!note]\n> <!-- llm-wiki-sync:callout:v1 icon=%F0%9F%94%8D color=blue_bg -->\n> <!-- llm-wiki-sync:callout:v1 icon=%F0%9F%94%8D color=blue_bg -->\n> body').status, "unsafe");
assert.equal(obsidianToNotionMarkdown('> [!note]\n> <!-- llm-wiki-sync:callout:v1 icon=bad%ZZ color=blue_bg -->\n> body').status, "unsafe");
const unknownWithLiteralCalloutText = '<callout icon="🔴" color="red_bg">\n\t`<callout icon="⚠️" color="yellow_bg">`\n\tbody\n</callout>';
const unknownWithLiteralCalloutTextPulled = notionToObsidianMarkdown(unknownWithLiteralCalloutText);
assert.equal(unknownWithLiteralCalloutTextPulled.status, "success");
assert.equal(obsidianToNotionMarkdown(unknownWithLiteralCalloutTextPulled.markdown).markdown, unknownWithLiteralCalloutText);
...
const notionToc = "<table_of_contents/>";
const notionTocMarker = "<!-- llm-wiki-sync:notion-toc:v1 -->";
const pulledToc = notionToObsidianMarkdown(notionToc);
assert.equal(pulledToc.status, "success");
assert.equal(pulledToc.markdown, notionTocMarker);
const pushedToc = obsidianToNotionMarkdown(notionTocMarker);
assert.equal(pushedToc.status, "success");
assert.equal(pushedToc.markdown, notionToc);
assert.equal(obsidianToNotionMarkdown(notionToObsidianMarkdown(notionToc).markdown).markdown, notionToc);
const tocInBody = notionToObsidianMarkdown("# Before\n\n<table_of_contents/>\n\n# After");
assert.equal(tocInBody.markdown, "# Before\n\n<!-- llm-wiki-sync:notion-toc:v1 -->\n\n# After");
const multipleTocs = notionToObsidianMarkdown("# One\n<table_of_contents/>\n# Two\n<table_of_contents/>\n# Three");
assert.equal(multipleTocs.markdown, "# One\n<!-- llm-wiki-sync:notion-toc:v1 -->\n# Two\n<!-- llm-wiki-sync:notion-toc:v1 -->\n# Three");
assert.equal(obsidianToNotionMarkdown(multipleTocs.markdown).markdown, "# One\n<table_of_contents/>\n# Two\n<table_of_contents/>\n# Three");
assert.equal(notionToObsidianMarkdown("Text <table_of_contents/>").status, "unsafe");
...
for (const literal of ["```md\n<!-- llm-wiki-sync:notion-toc:v1 -->\n```", "`<!-- llm-wiki-sync:notion-toc:v1 -->`"]) {
  const converted = obsidianToNotionMarkdown(literal);
  assert.equal(converted.status, "success");
  assert.equal(converted.markdown, literal);
}
for (const malformedTocMarker of ["<!-- llm-wiki-sync:notion-toc:v2 -->", "<!-- llm-wiki-sync:notion-toc:v1 broken -->", "Before <!-- llm-wiki-sync:notion-toc:v1 -->"]) {
  const converted = obsidianToNotionMarkdown(malformedTocMarker);
  assert.equal(converted.status, "unsafe");
  assert.equal(converted.markdown, malformedTocMarker);
}
const ordinaryHtmlComment = "<!-- unrelated comment -->";
assert.equal(obsidianToNotionMarkdown(ordinaryHtmlComment).markdown, ordinaryHtmlComment);
assert.equal(notionToObsidianMarkdown(ordinaryHtmlComment).markdown, ordinaryHtmlComment);
for (const unsupportedNotionBlock of ["<folder>x</folder>", "<details>x</details>", "<database>x</database>", "<synced_block>x</synced_block>"]) {
  assert.equal(notionToObsidianMarkdown(unsupportedNotionBlock).status, "unsafe");
}
