...
const CALLOUT_MARKER_PREFIX = "<!-- llm-wiki-sync:callout:v1 ";
const CALLOUT_MARKER_PATTERN = /^<!-- llm-wiki-sync:callout:v1 icon=([A-Za-z0-9%._~-]+) color=([A-Za-z0-9%._~-]+) -->$/;
const NOTION_TOC_MARKER = "<!-- llm-wiki-sync:notion-toc:v1 -->";
const NOTION_TOC_MARKER_PREFIX = "<!-- llm-wiki-sync:notion-toc:";
const STANDALONE_NOTION_TOC_PATTERN = /(^|\n)[ \t]*<table_of_contents\/>[ \t]*(?=\n|$)/g;
const STANDALONE_NOTION_TOC_MARKER_PATTERN = /(^|\n)[ \t]*<!-- llm-wiki-sync:notion-toc:v1 -->[ \t]*(?=\n|$)/g;
...
export function detectUnsupportedNotionConstructs(markdown: string): UnsupportedConstruct | null {
  ...
  const patterns: Array<{ pattern: RegExp; construct: string }> = [
    ...
    { pattern: /<(?:folder|details|summary|meeting-notes|columns|column|tabs|tab|database|synced_block|synced_block_reference|unknown|audio|video|file|pdf|embed|mention-[\w-]+)\b/i, construct: "unsupported Notion block" },
...
export function obsidianToNotionMarkdown(markdown: string): ConversionResult {
  ...
  if (hasMalformedNotionTocMarker(protectedInput.text)) return unsafeResult(markdown, "Malformed plugin Notion TOC marker cannot be sent to Notion.", warnings);
  ...
  while (index < lines.length) {
    if (lines[index].trim() === NOTION_TOC_MARKER) {
      output.push("<table_of_contents/>");
      tocCount += 1;
      index += 1;
      continue;
    }
    ...
    if (callout) {
      const preserved = readPreservedCallout(callout);
      if (preserved.error) return unsafeResult(markdown, preserved.error, warnings);
      if (preserved.value) {
        output.push(`<callout icon="${preserved.value.icon}" color="${preserved.value.color}">`);
        if (callout.title) output.push(`\t**${escapeGeneratedText(callout.title)}**`);
        output.push(...preserved.value.body.map((line) => `\t${line}`));
        output.push("</callout>");
        calloutCount += 1;
      } else if (!CALLOUTS[callout.type]) {
  ...
  const validation = validateOutbound(protectedInput.text, convertedBeforeRestore, calloutCount, tableCount, tocCount);
...
export function notionToObsidianMarkdown(markdown: string): ConversionResult {
  ...
  text = text.replace(STANDALONE_NOTION_TOC_PATTERN, (_match, prefix: string) => `${prefix}${NOTION_TOC_MARKER}`);
  if (/<table_of_contents\b/i.test(text)) return unsafeResult(markdown, "Malformed Notion table of contents block cannot be synced.", warnings);
  text = text.replace(/<callout\b([^>]*)>\n([\s\S]*?)\n<\/callout>/gi, (_match, attrsText: string, inner: string) => {
    ...
    return [header, `> ${createCalloutMarker(icon, color)}`, ...lines.map((line) => `> ${line}`)].join("\n");
  });
...
interface ObsidianCallout { type: string; title: string; body: string[]; end: number; }

function readObsidianCallout(lines: string[], start: number): ObsidianCallout | null {
...
function createCalloutMarker(icon: string, color: string): string {
  return `${CALLOUT_MARKER_PREFIX}icon=${encodeCalloutMarkerValue(icon)} color=${encodeCalloutMarkerValue(color)} -->`;
}

function readPreservedCallout(callout: ObsidianCallout): { value?: { icon: string; color: string; body: string[] }; error?: string } {
  const markerIndexes = callout.body.reduce<number[]>((indexes, line, index) => {
    if (line.includes(CALLOUT_MARKER_PREFIX)) indexes.push(index);
    return indexes;
  }, []);
  if (markerIndexes.length === 0) return {};
  if (markerIndexes.length !== 1 || markerIndexes[0] !== 0) return { error: "Preserved Notion callout marker is ambiguous or misplaced." };
  const match = CALLOUT_MARKER_PATTERN.exec(callout.body[0]);
  if (!match) return { error: "Preserved Notion callout marker is malformed." };
  const icon = decodeCalloutMarkerValue(match[1]);
  const color = decodeCalloutMarkerValue(match[2]);
  if (!icon || !color || !NOTION_BACKGROUND_COLORS.has(color) || createCalloutMarker(icon, color) !== callout.body[0]) {
    return { error: "Preserved Notion callout marker is malformed." };
  }
  return { value: { icon, color, body: callout.body.slice(1) } };
}

function encodeCalloutMarkerValue(value: string): string {
  return encodeURIComponent(value).replace(/[!'()*]/g, (character) => `%${character.charCodeAt(0).toString(16).toUpperCase()}`);
}

function decodeCalloutMarkerValue(value: string): string | null {
...
function detectUnsafeNotionCallout(text: string): UnsupportedConstruct | null {
  ...
  while ((match = pattern.exec(text)) !== null) {
    ...
    const icon = attributes.icon;
    const color = attributes.color;
    // An unfamiliar icon/color pair is valid and receives a lossless fallback.
    if (!icon || !color || !NOTION_BACKGROUND_COLORS.has(color)) return { construct: "malformed Notion callout" };
  }
...
function validateOutbound(input: string, output: string, callouts: number, tables: number, tocs: number): { ok: true } | { ok: false; error: string } {
  ...
  if (countNotionTocs(output) !== tocs) return { ok: false, error: "Converted Notion TOC count changed during validation." };
  ...
  if (countCallouts(recovered.markdown) !== callouts || countMarkdownTables(recovered.markdown) !== tables || countNotionTocMarkers(recovered.markdown) !== tocs) return { ok: false, error: "Generated structure disappeared during re-parse validation." };
...
function countNotionTocs(markdown: string): number { return (markdown.match(/<table_of_contents\/>/g) ?? []).length; }
function countNotionTocMarkers(markdown: string): number { return (markdown.match(STANDALONE_NOTION_TOC_MARKER_PATTERN) ?? []).length; }
function hasMalformedNotionTocMarker(markdown: string): boolean {
  return markdown.replace(STANDALONE_NOTION_TOC_MARKER_PATTERN, "$1").includes(NOTION_TOC_MARKER_PREFIX);
}
...
function containsUnknownNotionConstruct(markdown: string): boolean {
  if (/<(?:columns|column|tabs|tab|database|mention-|synced_block|synced_block_reference|audio|video|file|pdf|embed|unknown)\b/i.test(markdown)) return true;
